...
流程\
1采用低阈值*二值化*（找盆底）;2逐渐*增加阈值*（模拟水涨）;\
3初次接触的点为*边界*（找到分水岭）;4*修坝*以继续寻找高位分水岭。
缺点:受噪声影响大;容易过度分割
...
累积差值:将参考图像  $R(x, y)$  与序列中的每个后续图像进行比较。当当前图像中的像素与参考图像*不同*时，累积差分图像中每个像素的计数器会增加。在检查第  $t$  帧时，累积差分图像显示该像素与参考图像中对应像素的差异次数。 定义$R(x,y)=f(x,y,1)$
